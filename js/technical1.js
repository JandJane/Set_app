...
window.onload = function(){
    ...
    (function(){
        var date = new Date();
        div.innerHTML = 'Время: ' + (Math.floor(new Date().getTime() / 1000) - timme) + ' сек';
...
function show_results() {
	...
	if (score_post.value == 0){
	    results.innerHTML = '<h4 align = "center">Не расстраивайтесь!</h4> <strong>Ваш результат: </strong>' + 0 + ' очк. за ' + (Math.floor(new Date().getTime() / 1000) - timme) + ' cек';
	} else {
